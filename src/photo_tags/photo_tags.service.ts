...
export class PhotoTagsService {
  ...
  async create_photoTags(photoTag: PhotoTagsDto): Promise<PhotoTags> {
    ...
    const findTag = await this.PhotoTageRepo.findOne({where:{user:{id:photoTag.userId},post:{id:photoTag.postId}},relations:['user','post']});
    if (findTag) {
  ...
  async findPostTags(postId:number):Promise<PhotoTags[]>{
    const findTags=await this.PhotoTageRepo.find({where:{post:{id:postId}},relations:['post','user']});
    if(!findTags){
